...
module.exports = class extends Base {
  ...
  async infoAction() {
    ...
    const goodsEnum = await this.model('goods_enum').where({ goods_id: id }).find()
  ...
  async storeAction() {
    ...
    const goodsEnum = this.post('goodsEnum')
    const model = this.model('goods');
    ...
    if (id > 0) {
      ...
      // 更新商品高级查询
      let goodsEnumInfo = await this.model('goods_enum').where({ goods_id: id }).find()
      if(!think.isEmpty(goodsEnumInfo)){
        await this.model('goods_enum').where({
          goods_id: id
        }).update(goodsEnum);
      } else {
        await this.model('goods_enum').add({
          ...goodsEnum,
          goods_id: id
        });
      }
