...
module.exports = class extends Base {
  ...
  async indexAction() {
    ...
    for (const item of data.data) {
      item.goodsList = await this.model('order_goods').field(
        'goods_name,goods_aka,list_pic_url,number,goods_specifition_name_value,retail_price').where({
        order_id: item.id,
        is_delete: 0
      }).select();
      item.goodsCount = 0;
  ...
  async toDeliveryAction() {
    ...
    for (const item of data.data) {
      item.goodsList = await this.model('order_goods').field(
        'goods_name,list_pic_url,number,goods_specifition_name_value,retail_price').where({
        order_id: item.id
      }).select();
      item.goodsCount = 0;
      item.goodsList.forEach(v => {
  ...
  async detailAction() {
    ...
    data.goodsList = await this.model('order_goods').field(
        'id,product_id,goods_name,goods_aka,list_pic_url,number,goods_specifition_name_value,retail_price,goods_id')
      .where({
        order_id: data.id,
        is_delete: 0
      }).select();
  ...
  async getMianExpressAction() {
    ...
    const latestExpressInfo = await this.model('order_express').getMianExpress(orderId, senderInfo, receiverInfo,
      expressType);
  ...
  async rePrintExpressAction() {
    ...
    let order_sn = date.getFullYear() + _.padStart(date.getMonth(), 2, '0') + _.padStart(date.getDay(), 2, '0') + _
      .padStart(date.getHours(), 2, '0') + _.padStart(date.getMinutes(), 2, '0') + _.padStart(date.getSeconds(), 2,
        '0') + _.random(100000, 999999);
  ...
  async orderDeliveryAction() { // 发货api
    ...
    const logistic_code = this.get('logisticCode') || 0;
    const model = this.model('order');
    let currentTime = parseInt(new Date().getTime() / 1000);
    let expressName = '';
    if (method == 2) {
