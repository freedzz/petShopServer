...
const UserExt = require('./userExt.js');
...
module.exports = class extends Base {
  ...
  async checkoutAction() {
    ...
    if (!think.isEmpty(checkedAddress)) {
      ...
      checkedAddress.full_region = checkedAddress.province_name + checkedAddress.city_name + checkedAddress
        .district_name;
      for (const item of freightData) {
        if (item.number == 0) {
          continue;
        ...
        if (!think.isEmpty(ex)) {
          ...
          if (freight_type == 0) {
            if (item.number > groupData.start) { // 说明大于首件了
              freight_price = groupData.start * groupData.start_fee + (item.number - 1) * groupData
              .add_fee; // todo 如果续件是2怎么办？？？
            } else {
              freight_price = groupData.start * groupData.start_fee;
            }
          } else if (freight_type == 1) {
            if (item.goods_weight > groupData.start) { // 说明大于首件了
              freight_price = groupData.start * groupData.start_fee + (item.goods_weight - 1) * groupData
              .add_fee; // todo 如果续件是2怎么办？？？
            } else {
              freight_price = groupData.start * groupData.start_fee;
            }
